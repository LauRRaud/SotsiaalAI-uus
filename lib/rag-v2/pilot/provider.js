...
export async function providerCall({ stage, body, config, apiKey, transport = fetch }) {
  ...
  if (![usage.input, usage.output].every(x => Number.isSafeInteger(x) && x >= 0)) reject('provider_usage_unknown', 502);
  ...
  return { value, usage, requestId, timings };
  } catch (error) { throw Object.assign(error, { usage, requestId, timings }); }
}
